...
class DatPlotterApp:
    ...
    def _extract_segment_df(self, start_row, end_row, xcol, ycol, filter_col):
        ...
        if s > e:
            s, e = e, s
        cols_needed = {xcol, ycol, filter_col}
        missing = [c for c in cols_needed if c not in self.df.columns]
        if missing:
            raise ValueError(f"列不存在: {', '.join(missing)}")
        sub = self.df.iloc[s:e+1].copy()

        def convert_time_if_needed(df, col):
            if col == "Time Stamp (sec)" and self.convert_time_var.get():
                if self.fileopentime_ts is not None and self.fileopentime_dt is not None:
                    base_ts = self.fileopentime_ts
                    base_dt = self.fileopentime_dt
                    df[col] = pd.to_numeric(df[col], errors='coerce')
        ...
        convert_time_if_needed(sub, xcol)
        convert_time_if_needed(sub, ycol)
        ...
        df_clean = sub[[xcol, ycol, filter_col]].copy()
    ...
    def export_segments(self):
        ...
        if self.df is None:
            messagebox.showwarning("未加载数据", "请先打开 .dat 文件")
            return
        selections = self.seg_tree.selection()
        if not selections:
            messagebox.showwarning("未选择段", "请在列表中选择至少一个数据段")
            return
        xcol = self.x_combo.get()
        ycol = self.y_combo.get()
        filter_col = self.filter_col_cb.get()
        if not xcol or not ycol or not filter_col:
            messagebox.showwarning("列未选择", "请确保已选择 X、Y 和筛选列")
            return
        ...
        def _start_row(iid):
            try:
                ...
                return 0
        ordered_selections = sorted(selections, key=_start_row)

        segment_frames = []
        max_len = 0

        for idx, iid in enumerate(ordered_selections, start=1):
            vals = self.seg_tree.item(iid, "values")
            if len(vals) < 5:
                continue
            try:
                ...
                continue
            try:
                seg_df = self._extract_segment_df(start, end, xcol, ycol, filter_col)
            except Exception as exc:
                messagebox.showerror("导出失败", f"段 {start}-{end} 处理失败: {exc}")
                continue
            if seg_df is None or seg_df.empty:
                continue
            seg_df = seg_df.reset_index(drop=True)
            max_len = max(max_len, len(seg_df))
            # 计算筛选列均值用于列名识别
            mean_val = seg_df[filter_col].dropna().mean()
            if pd.isna(mean_val):
                label = f"seg{idx}"
            else:
                label = f"{mean_val:.4g}"
            label = str(label).replace("/", "_")
            segment_frames.append((seg_df, label))

        if not segment_frames or max_len == 0:
            messagebox.showinfo("无有效数据", "选中的数据段没有可导出的有效数据")
            return

        aligned_frames = []
        for idx, (seg_df, label) in enumerate(segment_frames, start=1):
            aligned = seg_df.reindex(range(max_len))
            aligned.columns = [
                f"{xcol}({label})",
                f"{ycol}({label})",
                f"{filter_col}({label})"
            ...
            # 在段与段之间插入一列空列作为分隔（最后一段后面不加）
            if idx < len(segment_frames):
                sep_col_name = f""
                aligned_frames.append(pd.DataFrame({sep_col_name: [np.nan] * max_len}))
